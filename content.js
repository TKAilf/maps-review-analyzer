...
class MapsReviewAnalyzer {
    ...
    observePageChanges() {
        const observer = new MutationObserver((mutations) => {
            ...
            mutations.forEach((mutation) => {
                // レビューコンテンツの変更を検出
                if (
                    mutation.target.closest("[data-review-id]") ||
                ...
                // 店舗情報の変更を検出
                if (
                    mutation.target.closest('[data-value="Reviews"]') ||
    ...
    parseReviewElement(element) {
        try {
            ...
            const hasPhotos =
                ...
                element.querySelectorAll('img[src*="googleusercontent"]')
                    .length > 0;
    ...
    detectPolarizedRatings() {
        ...
        if (extremeRatio > threshold && middleRatio < 0.2) {
            this.suspicionFactors.polarizedRatings = Math.min(
                extremeRatio * 100,
                90
            );
