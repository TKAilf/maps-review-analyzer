...
class PopupController {
  ...
  async toggleExtension() {
    try {
      ...
      if (response.success) {
        ...
        this.updateUI();
        this.showSuccess(
          newStatus ? "拡張機能を有効にしました" : "拡張機能を無効にしました"
        );
      } else {
        throw new Error(response.error);
      }
    ...
    try {
      ...
      const newSettings = {
        ...
        settings: {
          ...
          showDetailedAnalysis: document.getElementById("showDetailedAnalysis")
            .checked,
  ...
  async loadAnalysisHistory() {
    try {
      const response = await this.sendMessage({ type: "GET_ANALYSIS_HISTORY" });
